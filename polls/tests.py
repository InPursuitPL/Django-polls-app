...
def create_question_with_choice(question_text, days):
    ...
    Also creates choice with this question.
    """
    time = timezone.now() + datetime.timedelta(days=days)
    q = Question.objects.create(question_text=question_text, pub_date=time)
    q.choice_set.create(choice_text='Test choice', votes=0)
    return q
...
class QuestionViewTests(TestCase):
    ...
    def test_index_view_with_two_past_questions(self):
        ...
        """
        create_question_with_choice(question_text="Past question 1.", days=-30)
        create_question_with_choice(question_text="Past question 2.", days=-5)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerysetEqual(
            response.context['latest_question_list'],
    ...
    def test_index_view_with_past_question_no_choices(self):
        """
        Question without choices should not be displayed.
        """
        create_question_no_choice(question_text="Past question.", days=-5)
        response = self.client.get(reverse('polls:index'))
        self.assertContains(response, "No polls are available.")

    def test_index_view_with_past_question_with_choice(self):
        """
        Question with choice should be displayed.
        """
        create_question_with_choice(question_text="Past question.", days=-5)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerysetEqual(
            response.context['latest_question_list'],
            ['<Question: Past question.>']
